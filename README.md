...
- **시나리오 A (적성검사)**: 30문항 Yes/No 간접 행동 문항 → 9개 트랙 적성도 산출 → 게임 RPG 스타일 결과 카드 → Gemini AI 코멘트 → GAP 분석 연결
...
#### 시나리오 A — 적성검사 (진로 탐색)
1. 랜딩 페이지에서 **적성검사** 선택
2. 30문항 응답 (Yes / No 간접 행동 문항, 클릭 시 자동 다음 문항 이동)
...
### 시나리오 A — 적성검사 (구현 완료)
1. **30문항 Yes/No 간접 행동 문항** — 직접 선호가 아닌 행동·성향 기반 문항으로 편향 최소화
2. **자동 이동** — 응답 클릭 후 다음 문항 자동 이동
3. **트랙별 정규화 점수 산출** — 최대 원점수 기반 동적 정규화 (0~100점)
4. **게임 RPG 클래스 카드 결과 화면**
   - 트랙 그래디언트 배경 + 다크 오버레이
   - 아키타입 칭호 (예: 경험 설계자, 코드 아키텍트)
   - 어피니티 바 애니메이션 (0 → 점수% 슬라이드)
   - lore 텍스트 (트랙별 1인칭 세계관 서술)
5. **"이런 사람이 돼" 직업 아이덴티티 카드** — 3개 직업 × 아이콘·제목·한 줄 설명
6. **작업물 쇼케이스** — 3개 그래디언트 카드 (해당 직군이 만드는 결과물 시각화)
7. **초봉 가이드** — 고용노동부·업계 평균 기준 신입 연봉 범위 표시
8. **2순위 경로** — 아키타입·lore + 직업·취업처·연봉 요약 카드
9. **Gemini AI 맞춤 코멘트** — 결과 기반 자동 생성 (graceful fallback 내장)
10. **결과 판정 로직** — 저점 / 전체 동점 등 비정상 케이스 분리 처리
11. **GAP 분석 연결** — 결과 화면에서 시나리오 B로 바로 이동
