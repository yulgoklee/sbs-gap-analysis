...
- **시나리오 A (적성검사)**: 24문항 Likert 척도 → 트랙별 점수/레이더차트 → Gemini AI 코멘트 → GAP 분석 연결
...
#### 시나리오 A — 적성검사 (진로 탐색)
1. 랜딩 페이지에서 **적성검사** 선택
2. 24문항 응답 (Likert 5점 척도, 점수 클릭 시 자동으로 다음 문항 이동)
3. 결과 확인 — SVG 레이더차트 + 트랙별 점수 바 + Top 트랙 진로 정보
...
### 시나리오 A — 적성검사 (구현 완료)
1. **24문항 Likert 5점 척도** — 성향·관심사 18문항 + 희망·열망 6문항
2. **자동 이동** — 점수 클릭 400ms 후 다음 문항 자동 이동
3. **트랙별 정규화 점수 산출** — 최대 원점수 기반 공정 정규화 (0~100점)
4. **SVG 레이더차트** — 사용자 점수 + 평균 점선 비교 시각화
5. **점수 바** — 평균 이상/이하 구분, 트랙별 컬러
6. **Top 트랙 진로 정보** — 자격증·직업·취업처 안내
7. **Gemini AI 맞춤 코멘트** — 결과 기반 3~5문장 자동 생성
8. **결과 판정 로직** — 저점(top < 40) / 전체 동점 등 비정상 케이스 3종 분리 처리
9. **GAP 분석 연결** — 결과 화면에서 시나리오 B로 바로 이동
